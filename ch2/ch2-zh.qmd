...
-   關於 `ggplot2` 內附的 `mpg` 資料集，§2.2。

-   每張圖都有的三個關鍵元件：資料、美學與 geoms，§2.3。

-   如何利用美學為圖加入額外的變數，§2.4。

-   如何運用分面（faceting）所建立的小倍數圖，在圖中顯示額外的類別變數，§2.5。

-   各種你可以用來製作不同類型圖形的 geoms，§2.6。

-   如何修改座標軸，§2.7。

-   除了顯示圖形物件之外你還能對它做的事，例如把它存到磁碟，§2.8。
...
在第一張圖中，「blue」這個值被尺度轉換成了偏粉紅的顏色，並加上了圖例。在第二張圖中，點被賦予 R 的藍色。這是一個重要的技巧，你將在 §13.4.2 學到更多。關於顏色與其他美學所需的值，請參見 vignette("`ggplot2`-specs")。
...
你可能會想，何時該用分面、何時該用美學。你將在 §16.5 學到兩者各自的相對優缺點。
