...
- [`htmlwidgets`](https://www.htmlwidgets.org) provides a common framework for accessing web visualisation tools from R. Packages built on top of [`htmlwidgets`](https://www.htmlwidgets.org) include [`leaflet`](https://rstudio.github.io/leaflet/) for maps, [`dygraph`](https://rstudio.github.io/dygraphs/) for time series and [`networkD3`](https://christophergandrud.github.io/networkD3/) for networks.
...
Many other R packages, such as `vcd` [@meyer2007strucplot], `plotrix` [@lemon2008plotrix] and `gplots` [@warnes2005gplots], implement specialist graphics, but no others provide a framework for producing statistical graphics. A comprehensive list of all graphical tools available in other packages can be found in the CRAN Task Views at <https://cran.r-project.org/web/views/>.
...
- Coordinate systems and facetting control the position of elements of the plot. These are described in §13.7. Faceting is a very powerful graphical tool as it allows you to rapidly compare different subsets of your data. Different coordinate systems are less commonly needed, but are very important for certain types of data.
...
- **R**: This course does not assume you have used R before — we start from the beginning. Install it first; everything else builds on it. If you would like extra reading alongside the course, [R for Data Science](https://r4ds.hadley.nz) is a good companion.

- **RStudio**: a free and open source development environment for R. Other editors exist, but **this course requires RStudio** — the assignment workflow (the Knit button and the official template) assumes it. Download RStudio Desktop from <https://posit.co/download/rstudio-desktop>

---

- **R packages**: For assignments you only need `install.packages(c("tidyverse", "patchwork", "uuid", "rmarkdown", "knitr", "tinytex"))` — see the [course R page](https://yyliou.github.io/da/r.html). The longer list below covers every example used across all seventeen chapters; install it when you have time, and note that `sf` and `stars` can take a while to build:
