...
- **§8.1** — Setting plot and axis titles with `labs()`.
- **§8.2** — Adding text labels with `geom_text()` and `geom_label()`.
- **§8.3** — Building custom annotations using geoms and `annotate()`.
- **§8.4** — Direct labelling with `directlabels`, `ggforce`, and `gghighlight`.
...
- **extrafont** (<https://github.com/fbertran/extrafont>) — registers installed system fonts so all devices can use a common format.
...
- **Fitting labels inside bars or tiles.** The `ggfittext` package by David Wilkins provides geoms that automatically resize or trim text to fit within a specified bounding region.
...
`"smart.grid"` is a good default for scatterplots. For line plots and frequency polygons, other methods (e.g., `"last.points"`, `"angled.boxes"`) are more appropriate. See the directlabels website (<https://directlabels.netlify.app>) for a complete catalogue.
