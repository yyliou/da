...
使用預設的 `scale_size()`，與所對映的值（行星半徑）成比例的是每個點的**面積**，因此大行星與小行星之間的差異在視覺上被低估了。改用 `scale_radius()` 搭配 `limits = c(0, NA)`，把點的**半徑**線性對映到行星的半徑，使圓盤與行星在幾何上相似：
...
對比立即可見：木星的圓盤如今明顯比土星大得多，正確地反映出兩者的差異超過了地球的半徑。
...
`ggplot2` 支援 26 個相異的形狀，各以 0 到 25 的整數識別：

- 形狀 **0–14** 是空心的（只有邊框，無填色）。
- 形狀 **15–20** 是實心的（以 `colour` 美學填色）。
- 形狀 **21–25** 是填色形狀，會獨立地回應 `colour`（邊框）與 `fill` 兩個美學。
