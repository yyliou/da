...
With the default `scale_size()`, it is the *area* of each dot that is proportional to the mapped value (the planet's radius), so the difference between large and small planets is visually understated. Using `scale_radius()` with `limits = c(0, NA)` instead maps the dot's *radius* linearly to the planet's radius, which renders the discs geometrically similar to the planets:
...
The contrast is immediately visible: Jupiter's disc is now visibly much larger than Saturn's, correctly reflecting that the difference between the two exceeds the radius of Earth.
...
`ggplot2` supports 26 distinct shapes, each identified by an integer from 0 to 25:

- Shapes **0–14** are hollow (border only, no fill).
- Shapes **15–20** are solid (filled with the `colour` aesthetic).
- Shapes **21–25** are filled shapes that respond to both `colour` (border) and `fill` aesthetics independently.
