...
```
- `[deadline]` = 2026-06-12 23:59 (UTC+8) (Representing deadline time)
- `[no]` = 1 (Rrepresenting Assignment number, e.g., ex1, ps2,...)
- Timezone: UTC+8
- Root Directory Structures:
  |-- stu/            # Contains the official class roster (Excel/CSV)
  |-- da-ps/          # Contains the problem sets (html)
  `-- da-ex/
       |
...
1. Access the official class roster in the `stu/` folder to retrieve the full list of student IDs and names.
2. Cross-reference the submissions in `da-ex/ex[no]/`. 
...
4. **Plagiarism & Academic Integrity Check:**
   - **IF** the generated string from `uuid::UUIDgenerate()` in a student's file is duplicate with another student's or is an invalid/forged UUID
   - **THEN** both/all involved students receive Score = 0, Note = "Plagiarism"

---

## Step 3: Academic Content Grading
For submissions that pass all filters in Step 2, grade the content against the question set in `da-ps/`:
...
1. **Scoreboard Export (`ex-score.xlsx`):**
   - Create a new directory: `stu/ex[no]/`.
...
2. **Automated RMarkdown Analytics (`stat.Rmd` & `stat.pdf`):**
   - In the `stu/ex[no]/` directory, dynamically generate an RMarkdown file named `stat.Rmd`.
