...
- **§9.1**——把圖並排排列：`+`、`|` 與 `/` 運算子；`plot_layout()`；修改已拼裝的圖；以及 `plot_annotation()`。
- **§9.2**——用 `inset_element()` 把圖疊加成嵌入圖。
- **§9.3**——總結與進一步資源。
...
當沒有明確指定布局時，patchwork 套用與 `facet_wrap()` 相同的啟發式來決定列數與欄數。加入三張圖會產生留有一個空格的 2 × 2 格網；四張圖則產生 2 × 2 的格網：
