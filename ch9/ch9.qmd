...
- **§9.1** — Laying out plots side by side: the `+`, `|`, and `/` operators; `plot_layout()`; modifying assembled plots; and `plot_annotation()`.
- **§9.2** — Overlaying plots as insets with `inset_element()`.
- **§9.3** — Summary and further resources.
...
When no layout is specified explicitly, patchwork applies the same heuristic used by `facet_wrap()` to determine the number of rows and columns. Adding three plots produces a 2 × 2 grid with one empty cell; four plots produce a 2 × 2 grid:
