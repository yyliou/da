---
title: "Data Analysis"
subtitle: "Chapter 5: Statistical summaries"
author: "Yu-You Liou"
...
**`geom_boxplot()`** — the box-and-whisker plot shows the median, Q1 and Q3 as a box, with whiskers extending to the most extreme observations within 1.5×IQR of the box; points beyond are flagged as potential outliers. It occupies minimal space and scales well to many groups.
...
`stat_density2d()` estimates the bivariate density and can be combined with any geom capable of displaying a third dimension (see §5.7). This approach produces smooth contours but involves more computational overhead than binning.

**Strategy 6: Add a summary layer**

Adding `geom_smooth()` on top of a scatterplot draws a smooth conditional mean, guiding the eye toward the underlying trend without removing the raw data. Other summary geoms (see §5.6) can serve a similar role.
...
For **interactive 3D surfaces** (true perspective views, rotatable plots), the `rgl` package (<https://dmurdoch.github.io/rgl/>) provides the necessary tools beyond ggplot2's scope.
