...
- **§6.1**——以 `geom_polygon()` 製作的、以多邊形為基礎的地圖，最簡單的方法。
- **§6.2**——以 `geom_sf()` 製作的現代簡單要素（sf）地圖。
- **§6.3**——地圖投影與座標參考系統。
- **§6.4**——以程式化方式處理 sf 資料結構。
- **§6.5**——使用衛星影像、以光柵為基礎的地圖。

# 多邊形地圖

## 多邊形地圖

在 ggplot2 中繪製地圖最直接的途徑，是把地理邊界表示為普通的多邊形，並以 `geom_polygon()` 呈現它們。可自 CRAN 安裝的 `maps` 套件，提供了可透過 `ggplot2::map_data()` 存取的邊界資料。
...
`coord_quickmap()` 是一個快速的線性近似。對於高精度的製圖——尤其是大區域、極區，或出版用的製圖產品——`geom_sf()` / `coord_sf()` 工作流程（§6.2）提供了以正規地圖投影為基礎的嚴謹解法。
...
`coord_sf()` 處理地圖投影；它的角色將在 §6.3 完整討論。
...
關於 R 中地圖投影的詳盡論述，請見 *Geocomputation with R*（<https://r.geocompx.org/>）。
...
- `st_dimension()`——回傳每個幾何的拓撲維度：點為 0，線串為 1，多邊形為 2（與座標是 XY 或 XYZ 無關）。
...
**關於互動式 3D 地圖的提醒：** 至於超出 ggplot2 範疇的真正透視視圖或可旋轉 3D 曲面視覺化，`rgl` 套件（<https://dmurdoch.github.io/rgl/>）是標準工具。
...
- **osmdata**（<https://docs.ropensci.org/osmdata/>）——與 OpenStreetMap API 介接，以擷取細緻的向量資料：街區層級的個別街道、建築、公園與興趣點（Eugster & Schlesinger 2010）。
