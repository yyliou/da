...
- **§6.1** — polygon-based maps with `geom_polygon()`, the simplest approach.
- **§6.2** — modern simple features (sf) maps with `geom_sf()`.
- **§6.3** — map projections and coordinate reference systems.
- **§6.4** — working programmatically with sf data structures.
- **§6.5** — raster-based maps using satellite imagery.

# Polygon maps

## Polygon maps

The most straightforward path to drawing a map in ggplot2 is to represent geographic boundaries as ordinary polygons and render them with `geom_polygon()`. The `maps` package, available from CRAN, provides boundary data accessible through `ggplot2::map_data()`.
...
`coord_quickmap()` is a fast linear approximation. For high-accuracy mapping — especially large regions, polar areas, or published cartographic products — the `geom_sf()` / `coord_sf()` workflow (§6.2) provides a rigorous solution based on proper map projections.
...
`coord_sf()` handles the map projection; its role is discussed fully in §6.3.
...
For a thorough treatment of map projections in R, see *Geocomputation with R* (<https://r.geocompx.org/>).
...
- `st_dimension()` — returns the topological dimension of each geometry: 0 for points, 1 for linestrings, 2 for polygons (regardless of whether coordinates are XY or XYZ).
...
**Note on interactive 3D maps:** For true perspective-view or rotatable 3D surface visualisations beyond ggplot2's scope, the `rgl` package (<https://dmurdoch.github.io/rgl/>) is the standard tool.
...
- **osmdata** (<https://docs.ropensci.org/osmdata/>) — interfaces with the OpenStreetMap API to retrieve granular vector data: individual streets, buildings, parks, and points of interest at the city block level (Eugster & Schlesinger 2010).
